...
- **Base**: the repo's integration branch. Confirm it — `clay-base` uses `dev`; most repos
  use `main`. If unsure: `git -C <repo> symbolic-ref --short refs/remotes/origin/HEAD`.
...
- **Claude Code** — the folder-trust dialog and (for repos whose `CLAUDE.md` uses `@file`
  includes, e.g. `clay-base`'s `@AGENTS.md`) the external-includes approval. Pre-seed both
